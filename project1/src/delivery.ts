...
function calcDiscount(code:string):number{
    switch(code){
        case 'TRYNEW':
            return 100;
        case 'BINGE250':
...
function isLate(check:string):number{
    if(check=='Y'){
        return 0;
    }
    else{
...
function distanceFee(distance:number):number{
    if(distance<2){
        return 0;
    }
    else if(distance < 5){
...
function weatherFee(check:string):number{
    if(check=='Y'){
        return 30;
    }
    else{
...
function calcFee(logs:Details[]):void{
